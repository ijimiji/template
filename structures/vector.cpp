...
template <class T> Vector<T>::Vector(unsigned int size, const T &initial) {
    my_size = size;
    my_capacity = size;
    buffer = new T[size];
...
template <class T> void Vector<T>::pop_back() { my_size--; }
...
template <class T> void Vector<T>::resize(unsigned int size) {
    reserve(size);
    my_size = size;
}
